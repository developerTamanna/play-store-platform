const Testimonials = () => {
  const users = [
    {
      name: "Tamanna Akter",
      img: "https://i.pravatar.cc/100?img=1",
      rating: 5,
      message:
        "This app is super helpful and easy to use. Highly recommended for everyone!",
    },
    {
      name: "Lima Khatun",
      img: "https://i.pravatar.cc/100?img=2",
      rating: 4,
      message: "Nice UI and smooth performance. Waiting for new features!",
    },
    {
      name: "Rahim Uddin",
      img: "https://i.pravatar.cc/100?img=3",
      rating: 5,
      message: "I love how user-friendly this app is. It made my life easier!",
    },
  ];

  return (
    <section className="py-12 px-4 md:px-10">
      <h2 className="text-3xl font-bold mb-4 text-center">
        What Our Users <span className="text-primary">Say</span>
      </h2>
      <p className="text-gray-500 text-sm w-1/2 mx-auto mt-2 text-center mb-10">
        Our users consistently praise our product's ease of use and powerful
        features. They say it has made a real difference in their work and daily
        lives.
      </p>
      <div className="grid gap-6 md:grid-cols-3">
        {users.map((user, index) => (
          <div
            key={index}
            className="bg-white p-6 rounded-2xl shadow hover:shadow-md transition"
          >
            <div className="flex items-center space-x-4 mb-4">
              <img
                src={user.img}
                alt={user.name}
                className="w-12 h-12 rounded-full"
              />
              <div>
                <h4 className="text-lg font-semibold text-gray-800">
                  {user.name}
                </h4>
                <p className="text-yellow-400">
                  {"★".repeat(user.rating) + "☆".repeat(5 - user.rating)}
                </p>
              </div>
            </div>
            <p className="text-gray-600">{user.message}</p>
          </div>
        ))}
      </div>
    </section>
  );
};

export default Testimonials;
